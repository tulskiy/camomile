...
#if LIBAVFORMAT_VERSION_MAJOR < 53 && CONFIG_SHARED && HAVE_SYMVER && !ANDROID
    ...
    for(;;){
        ...
        if (ret < 0) {
            if (!pktl || ret == AVERROR(EAGAIN)) {
            	//LOGE("1 av_read_packet returning %d again?=%d", ret, ret == AVERROR(EAGAIN));
                return ret;
            }
        ...
        if(!pktl && (st->codec->codec_id != CODEC_ID_PROBE ||
                     !st->probe_packets)) {
            //LOGE("2 av_read_packet returning %d", ret);
        	return ret;
        }
    ...
    for(;;) {
        ...
        if (st) {
            ...
            if (ret < 0) {
                ...
                //LOGE("return no more packets");
                return ret;
            }
            //LOGW("av_read_packet ret=%d", ret);
...
//                return -1;
    ...
    //LOGW("av_read_frame_internal return 0");
    return 0;
}
...
/*            if(!st->time_base.num)
    ...
    for(;;) {
        ...
        //LOGE("av_read_frame_internal=%d", ret);
        if (ret < 0) {
            /* EOF or error */
        ...
        if(st->codec_info_nb_frames>1) {
            if (st->time_base.den > 0 && av_rescale_q(codec_info_duration[st->index], st->time_base, AV_TIME_BASE_Q) >= ic->max_analyze_duration){
                av_log(ic, AV_LOG_WARNING, "max_analyze_duration reached %d\n", ic->max_analyze_duration);
                break;
            }
