...
static int mp3_read_probe(AVProbeData *p)
{
    ...
    if(ff_id3v2_match(buf0, ID3v2_DEFAULT_MAGIC)) {
        //LOGE("buf len=%d %x", ff_id3v2_tag_len(buf0), ff_id3v2_tag_len(buf0));
    ...
    //LOGW("mp3_read_probe first_frames=%d max_frames=%d", first_frames, max_frames);
...
static int mp3_parse_vbr_tags(AVFormatContext *s, AVStream *st, int64_t base)
{
    ...
    //LOGW("mp3_parse_vbr_tags ftell=%llx", url_ftell(s->pb));

    v = get_be32(s->pb);
    if(ff_mpa_check_header(v) < 0) {
    	//LOGE("ff_mpa_check_header(v) < 0, v=%x", v);
    	url_fseek(s->pb, base, SEEK_SET);

    	// Try to resync if some padding exists.
    	for(int i = 0; i < MPA_MAX_CODED_FRAME_SIZE; i++) {
    		sync_probe = get_byte(s->pb);
    		
    		//LOGW("=syncing=%x ftell=%llx", (unsigned)sync_probe, url_ftell(s->pb));

    		if(sync_probe == 0xFF) {
	    		//LOGW("sync_probe == 0xFF");
...
found_header:
    ...
    if(v == MKBETAG('X', 'i', 'n', 'g') || v == MKBETAG('I', 'n', 'f', 'o')) {
        ...
        if(v & 0x1) {
        	...
        	//LOGE("v & 0x1 frames=%d", frames);
        }
        if(v & 0x2) {
            size = get_be32(s->pb);
            //LOGE("v & 0x2 size=%d", size);
        }

		if(s->enc_delay == 0 && s->enc_padding == 0) {
			if(v & VBR_TOC_FLAG) {
				url_fskip(s->pb, 100);
			}
			if(v & VBR_QUALITY_FLAG) {
				url_fskip(s->pb, 4);
			}
			url_fskip(s->pb, 21);
			
			char buf[3];
			get_buffer(s->pb, buf, 3);
			int enc_delay   = ((int)buf[0] << 4) + (buf[1] >> 4);
			int enc_padding = ((int)(buf[1] & 0xF) << 8) + buf[2];
			
			if(enc_delay >= 0 && enc_delay <= 1152 * 2 && enc_padding >= 0 && enc_padding <= 4 * 1152) {
				//LOGE("LAME enc_delay=%d enc_padding=%d", enc_delay, enc_padding);
				//av_metadata_set2(&s->metadata, "enc_delay", val, 0);
				s->enc_delay = enc_delay;
				s->enc_padding = enc_padding;
			}
		}
    ...
    if(v == MKBETAG('V', 'B', 'R', 'I')) {
        ...
        if(get_be16(s->pb) == 1) {
            ...
            size = get_be32(s->pb); // FIXED: # of bytes comes first in VBRI header.
            frames = get_be32(s->pb); 
        }
    }
    ...
    //LOGW("frames=%d size=%d sr=%d", frames, size, c.sample_rate);
                           ...
                           AVFormatParameters *ap)
{
    ...
    //LOGE("mp3_read_header, ftell=%lld", url_ftell(s->pb));

    st = av_new_stream(s, 0);
    if (!st)
        return AVERROR(ENOMEM);
    
    //LOGE("mp3_read_header2, ftell=%lld", url_ftell(s->pb));
    ...
    //ff_id3v2_skip(s, ID3v2_DEFAULT_MAGIC);
    av_metadata_set2(&s->metadata, "title", "", 0);
    ...
    //LOGE("mp3_read_header4, ftell=%llx", url_ftell(s->pb));
...
static int mp3_read_packet(AVFormatContext *s, AVPacket *pkt)
{
    ...
    //LOGW("mp3_read_packet===============================, pb->pos=%lld ftell=%lld", s->pb->pos, url_ftell(s->pb));

    size= MP3_PACKET_SIZE;

    ret= av_get_packet(s->pb, pkt, size);
    
    //LOGW("av_get_packet=%d %x", ret, ret);
