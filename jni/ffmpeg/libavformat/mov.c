...
// NOTE: this will parse only first entry (iTunSMPB) in mean/name/data collection.
static int mov_read_itunsmpb(MOVContext *c, ByteIOContext *pb, MOVAtom atom, int tag, unsigned data_size) {
	int str_size;
	char str[1024];
	
	if(tag == MKTAG('m','e','a','n')) { // Mean
		get_be32(pb); // Flag

		url_fskip(pb, data_size - 12); // 12 => size(4) + tag(4) + flag(4)
		...
		if(tag == MKTAG('n','a','m','e')) {
			str_size = data_size - 12;
			get_buffer(pb, str, str_size);
			str[str_size] = 0;
			//LOGE("name=%s", str);
			if(strcmp(str, "iTunSMPB") == 0) {
				//LOGE("got iTunSMPB");
				...
				if(tag == MKTAG('d','a','t','a')) {
					...
					str_size = FFMIN(sizeof(str) - 1, str_size);
					get_buffer(pb, str, str_size);
					
					//LOGE("got str_size=%d str=%s", str_size, str);
					char* val = str;
					char *parsed;
					int enc_delay = 0, enc_padding = 0;
		
					// Skip first number.
					strtol(val, &parsed, 16);
					if(parsed > val) {
						// Parse second number
						val = parsed;
						enc_delay = strtol(val, &parsed, 16);
						if(parsed > val) {
							val = parsed;
							enc_padding = strtol(val, &parsed, 16);
						}
					}
...
static int mov_read_udta_string(MOVContext *c, ByteIOContext *pb, MOVAtom atom)
{
    ...
    //LOGE("mov_read_udta_string atom.size=%lld type=%x", atom.size, atom.type);
    ...
    if (c->itunes_metadata && atom.size > 8) {
        ...
        if(atom.type ==  MKTAG( '-','-','-','-')) {
        	...
        	return mov_read_itunsmpb(c, pb, atom, tag, data_size);
    ...
    if (atom.size < 0) {
        //LOGE("atom.size < 0");
    	return 0; // MaxMP mod - ignore "bad" atom.
    }
...
static int mov_read_dref(MOVContext *c, ByteIOContext *pb, MOVAtom atom)
{
    ...
    if (entries >= UINT_MAX / sizeof(*sc->drefs)) {
        LOGE("mov_read_dref -1");
    	return -1;
    }
    sc->drefs = av_mallocz(entries * sizeof(*sc->drefs));
    if (!sc->drefs) {
    	LOGE("mov_read_dref AVERROR(ENOMEM)");
        ...
        if (dref->type == MKTAG('a','l','i','s') && size > 150) {
            ...
            for (type = 0; type != -1 && url_ftell(pb) < next; ) {
                ...
                if (type == 2) { // absolute path
                    ...
                    if (!dref->path) {
                    	LOGE("mov_read_dref AVERROR(ENOMEM) 2");
                        return AVERROR(ENOMEM);
                    }
                    ...
                    if (!dref->dir) {
                        LOGE("mov_read_dref AVERROR(ENOMEM) 3");
                    	return AVERROR(ENOMEM);
                    }
    ...
    if (tag == MP4DecConfigDescrTag) {
        ...
        if (tag == MP4DecSpecificDescrTag) {
            ...
            if((uint64_t)len > (1<<30)) {
                LOGE("ff_mov_read_esds -1");
            	return -1;
            }
            st->codec->extradata = av_mallocz(len + FF_INPUT_BUFFER_PADDING_SIZE);
            if (!st->codec->extradata) {
            	LOGE("ff_mov_read_esds ENOMEM");
                return AVERROR(ENOMEM);
            }
...
static int mov_read_ftyp(MOVContext *c, ByteIOContext *pb, MOVAtom atom)
{
    ...
    if (!comp_brands_str) {
        LOGE("mov_read_ftyp AVERROR(ENOMEM)");
    	return AVERROR(ENOMEM);
    }
...
static int mov_read_extradata(MOVContext *c, ByteIOContext *pb, MOVAtom atom)
{
    ...
    if(size > INT_MAX || (uint64_t)atom.size > INT_MAX) {
        LOGE("mov_read_extradata -1");
    	return -1;
    }
    buf= av_realloc(st->codec->extradata, size);
    if(!buf) {
        LOGE("mov_read_extradata -1 2");
    	return -1;
    }
...
static int mov_read_freeform(MOVContext *c, ByteIOContext *pb, MOVAtom atom)
{
    MOVStreamContext *sc;
    return 0;
}
...
static int mov_read_header(AVFormatContext *s, AVFormatParameters *ap)
{
    ...
    s->enc_padding = mov->enc_padding; // MaxMP
    s->enc_delay = mov->enc_delay; // MaxMP
