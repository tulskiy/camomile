...
void ff_id3v2_read(AVFormatContext *s, const char *magic)
{
    ...
    //LOGW("ff_id3v2_read ftell=%llx", url_ftell(s->pb));

    initial_off = url_ftell(s->pb);
    ret = get_buffer(s->pb, buf, ID3v2_HEADER_SIZE);
    
    //LOGW("ff_id3v2_read2 ftell=%llx", url_ftell(s->pb));
    
    if (ret != ID3v2_HEADER_SIZE)
        return;
    if (ff_id3v2_match(buf, magic)) {
    	
    	//LOGW("ff_id3v2_read3 match ftell=%llx", url_ftell(s->pb));
...
//        len = ff_id3v2_tag_len(buf);
        
        //int64_t before = url_ftell(s->pb);
        ff_id3v2_parse(s, len, buf[3], buf[5]);
	        
        //url_fskip(s->pb, len);
        //av_metadata_set2(&s->metadata, "title", "", 0);
        
		//LOGE("size skept=%llx len=%d len=%x", url_ftell(s->pb) - before, len, len);        
...
//        uint8_t flags = buf[5];
        ...
        //LOGW("ff_id3v2_read4 ftell=%lld", url_ftell(s->pb));
    } else {
        //LOGW("ff_id3v2_read no match seek to initial_off=%llx", initial_off);
        url_fseek(s->pb, initial_off, SEEK_SET);
    }
}
...
void ff_id3v2_parse(AVFormatContext *s, int len, uint8_t version, uint8_t flags)
{
    ...
    if (isv34 && flags & 0x40) /* Extended header present, just skip over it */ {
        //LOGE("url skip %d", get_size(s->pb, 4));
