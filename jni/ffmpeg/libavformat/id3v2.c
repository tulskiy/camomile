...
int ff_id3v2_skip(AVFormatContext *s, const char *magic) {
    int len, ret;
    int64_t initial_off;
    uint8_t buf[ID3v2_HEADER_SIZE];
    
    initial_off = url_ftell(s->pb);
    ...
    if (ff_id3v2_match(buf, magic)) {
        ...
        url_fseek(s->pb, initial_off, SEEK_SET);
        return 0;
    }
}
...
static void read_ttag(AVFormatContext *s, ByteIOContext *pb, int taglen, const char *key)
{
    ...
    // MaxMP: parse only COMM/COM tag.
    if(strcmp(key, "COMM") == 0 || strcmp(key, "COM") == 0) {
    	...
    	//LOGW("got COM, key=%s val=%s", key, val);

    	if(strcmp(key, "engiTunSMPB") == 0) {
    		//LOGW("get iTunes gapless=>%s", val);
    		char *parsed;
		    ...
		    if(enc_delay >= 0 && enc_delay <= 1152 * 2 && enc_padding >= 0 && enc_padding <= 4 * 1152) {
		    	//LOGW("itunes enc_delay=%d enc_padding=%d", enc_delay, enc_padding);
		    	s->enc_delay = enc_delay;
		    	s->enc_padding = enc_padding;
		    }
    	}

    }// else if (!(strcmp(key, "TCON") && strcmp(key, "TCO"))
...
void ff_id3v2_parse(AVFormatContext *s, int len, uint8_t version, uint8_t flags)
{
    ...
    //LOGW("===============ff_id3v2_parse() len=%d ftell=%llx", len, url_ftell(s->pb));
...
//    }
    ...
    while (len >= taghdrlen) {
        ...
        //LOGW("ff_id3v2_parse 4 len=%d taghdrlen=%d isv34=%d version=%d", len, taghdrlen, isv34, version);
        ...
        int offset = taghdrlen + tlen;
        //LOGW("offset=%d taghdrlen=%d tlen=%d tag=%s", offset, taghdrlen, tlen, tag);
        if(offset >= 0) {  // MaxMP: guard against bad tags, garbage in tlen (as int is used).
        	len -= offset;        
        }
        ...
        //} else if (tag[0] == 'C') { // MaxMP: interested only in COM/COMM tags.
        } else if(isv34 && strcmp(tag, "COMM") == 0 || !isv34 && strcmp(tag, "COM") == 0) {
        	//LOGW("GET tag %s", tag);
            if (unsync || tunsync) {
                int i, j;
