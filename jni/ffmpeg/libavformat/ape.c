...
static int ape_read_header(AVFormatContext * s, AVFormatParameters * ap)
{
    ...
    /* MaxMP: Skip any leading junk such as id3v2 tags */
    ff_id3v2_skip(s, ID3v2_DEFAULT_MAGIC);
    ape->junklength = url_ftell(s->pb);
    ...
    for (i = 1; i < ape->totalframes; i++) {
        ape->frames[i].pos      = ape->seektable[i] + ape->junklength; // MaxMP
    ...
    if (!url_is_streamed(pb)) {
        ff_ape_parse_tag(s);
        url_fseek(pb, ape->junklength, SEEK_SET); // MaxMP
    }
...
static int ape_read_packet(AVFormatContext * s, AVPacket * pkt)
{
    ...
    //LOGW("ape frame=%d", ape->currentframe);
