...
#define  LOG_TAG    "mpegaudio_parser.c"
...
int ff_mpa_decode_header(AVCodecContext *avctx, uint32_t head, int *sample_rate, int *channels, int *frame_size, int *bit_rate)
{
    ...
    //LOGW("ff_mpa_decode_header head=%x", head);
    ...
    if (ff_mpegaudio_decode_header(s, head) != 0) {
    	//LOGW("ff_mpegaudio_decode_header != 0");
    	return -1;
    }
        ...
        if(s->frame_size){
            ...
            while(i<buf_size){
                ...
                if(ret > 0 && frame_size > 0) {
                	// maxmp Fix -> set sample_rate from the first frame available, as next frames can contain wrong overal sample rate if file is VBR. 
                	//LOGE("set first sample_rate fix=%d", sr);
